...
def normalize_city(city: str) -> str:
    ...
    # Крок 1: Область на початку рядка → видаляємо її і кому/пробіл після неї.
    # "Харківська область м.Південне" → "м.Південне"
    # "Сумська обл., м. Конотоп"     → "м. Конотоп"
    # "Київська область, Коцюбинське" → "Коцюбинське"
    city = re.sub(
        r"^\S+(?:ська|зька|цька|нська|рська)\s+(?:область|обл\.?)\s*[,\s]*",
        "", city, flags=re.IGNORECASE,
    )
    # Район на початку після області
    # "Бориспільський р-н м.Бориспіль" → "м.Бориспіль"
    city = re.sub(
        r"^\S+(?:ський|зький|цький|нський|рський)\s+(?:район|р-н)\s*[,\s]*",
        "", city, flags=re.IGNORECASE,
    )

    # Крок 2: Типові префікси міста.
    # Однолітерні (м, с) вимагають крапку — інакше "Миколаїв" → "иколаїв"
    city = re.sub(
        r"^(смт\.?\s*|м\.\s*|с\.\s*|сел\.\s*|місто\s+|селище\s+|село\s+|с-ще\.?\s*)",
        "", city, flags=re.IGNORECASE,
    )

    # Видаляємо все в дужках: "Петриківка (Дніпропетровська область)" → "Петриківка"
    city = re.sub(r"\s*\(.*?\)", "", city)
    # Видаляємо вулицю/адресу після коми
    city = re.sub(r"\s*,.*$", "", city)
    # Видаляємо " <прикметник> область/обл/район/р-н ..." в кінці
    city = re.sub(r"\s+\S+(?:ська|зька|цька|нська|рська)\s.*$", "", city, flags=re.IGNORECASE)
    city = re.sub(r"\s+(область|обл\.?|район|р-н|вул\.?|вулиця).*$", "", city, flags=re.IGNORECASE)
    return city.strip()
